...
main(int argc, char** argv) 
{
    ...
    for(i=0; i<N_INT; i++)
    {
        ...
        if(me == 0){
            for(r = 1; r < nproc ;r++)
                MPI_Send(&n,1, MPI_FLOAT,r,0,MPI_COMM_WORLD);
        }
        else{
            MPI_Recv(&n,1, MPI_FLOAT,0,0,MPI_COMM_WORLD);
        }
    ...
    //MPI_Reduce(integral, total, Ns_INT, MPI_FLOAT, MPI_SUM, root, MPI_COMM_WORLD);
    if(me == 0){
        for(r = 0 ; r < N_INT ;r++){
            total[r] = integral[r];
        }
